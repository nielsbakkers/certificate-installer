...
sudo apt-get install $webinstall
...
perl -p -i -e 's/\.\/demoCA/\.\/$ENV{dir}/g' /usr/lib/ssl/openssl.cnf
...
openssl sudo ca -batch -in $dirL/certreq-$domainname.csr -out $dirL/cert-$domainname.pem 
